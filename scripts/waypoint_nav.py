...
class WaypointNav(object):
    ...
    def __init__(self) -> None:
        ...
        self.object_nav = rospy.Service('waypoint_nav/', StringBool, self.handler)

        rospy.loginfo("/waypoint_nav service running")
    ...
    def handler(self, req):
        ...
        # Get waypoint info
        try:
            waypoint_pose = rospy.get_param(f'waypoints/{req.msg}')
        except KeyError:
            rospy.logerr(f'waypoint_named "{req.msg}" not found')
            return False
        
        goal = self.build_goal(waypoint_pose)
