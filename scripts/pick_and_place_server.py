...
class PickAndPlaceServer(object):
	...
	def grasp_object(self, object_pose):
		...
		# Get position of table in the 'map' frame
		table_position = self.get_object_pose("table")

		# Set general table pose if request fails
		if not table_position.success:
			table_pose.pose.position.x = 1.1
		
		pose_stamped = PoseStamped()
		pose_stamped.header.frame_id = 'map'
		pose_stamped.header.stamp = rospy.Time.now()
		pose_stamped.pose = table_position.pose
		...
		table_pose.pose.position = pose_transformed.pose.position
